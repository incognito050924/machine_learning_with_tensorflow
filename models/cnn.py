...
def simple_cnn(inputs, num_classes, is_training=True, dropout_keep_prob=0.8, min_depth=16,
    ...
    with tf.variable_scope(scope, 'SimpleCNN', [inputs, num_classes], reuse=reuse) as scope:
        with arg_scope([slim.batch_norm, slim.dropout], is_training=is_training):
            with arg_scope([slim.conv2d], weights_initializer=weight_initializer('trunc_norm', stddev=0.01),
                           normalizer_fn=slim.batch_norm , normalizer_params=batch_norm_params,
                           activation_fn=lrelu, stride=1, padding='SAME'):
                net = slim.conv2d(inputs, depth(32), [5, 5], scope='Conv2d_0a_5x5')
                net = slim.max_pool2d(net, [2, 2], scope='Max_pool_0b_2x2')
                net = slim.conv2d(net, depth(64), [5, 5], scope='Conv2d_1a_5x5')
                net = slim.max_pool2d(net, [2, 2], scope='Max_pool_1b_2x2')
                net = slim.flatten(net, scope='Flatten_3a')
                net = slim.fully_connected(net, 1024
                                           , activation_fn=lrelu
                                           , weights_initializer = weight_initializer('trunc_norm', stddev=0.01)
                                           , normalizer_fn=slim.batch_norm
                                           , normalizer_params=batch_norm_params
                                           , scope='Dense_4a')
                net = slim.dropout(net, keep_prob=dropout_keep_prob, scope='Dropout')
                out = slim.fully_connected(net, num_classes
